...
const DetailOne = (props: DetailProps) => {
    const router = useRouter();
    const ref = useRef<HTMLDivElement>(null);
    const { product } = props;
    const [qty, setQty] = useState<number>(1);
    const [qty2, setQty2] = useState<number>(1);
    ...
    useEffect(() => {
        ...
        let toggle = ref.current!.querySelector(
            '.variation-toggle'
        )! as HTMLElement;
    ...
    const scrollHandler = () => {
        if (router.pathname.includes('/product/default')) {
            let stickyBar = ref.current!.querySelector(
                '.sticky-bar'
            )! as HTMLElement;
            if (
    ...
    const onChangeQty2 = (current: number) => {
        setQty2(current);
    };
    ...
    const onCartClick = (e: MouseEvent<HTMLAnchorElement>, index = 0) => {
        e.preventDefault();
